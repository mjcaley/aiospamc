...
@pytest.mark.asyncio
async def test_symbols_connection_refused(event_loop, unused_tcp_port):
...
@pytest.mark.asyncio
async def test_symbols_invalid_response(event_loop, unused_tcp_port):
...
@pytest.mark.asyncio
async def test_symbols_valid_request(event_loop, unused_tcp_port):
    mock = MockServer(b'SPAMD/1.5 0 EX_OK\r\n')
    server = await asyncio.start_server(mock.handle_connection,
                                        host=HOST,
                                        port=unused_tcp_port,
    ...
    assert mock.requests[0].decode() == 'SYMBOLS SPAMC/1.5\r\nContent-length: {}\r\n\r\n{}'.format(len(GTUBE.encode()), GTUBE)

@pytest.mark.asyncio
async def test_symbols_compress_header_request(event_loop, unused_tcp_port):
    mock = MockServer(b'SPAMD/1.5 0 EX_OK\r\n')
    server = await asyncio.start_server(mock.handle_connection,
                                        host=HOST,
                                        port=unused_tcp_port,
...
@pytest.mark.asyncio
async def test_symbols_user_header_request(event_loop, unused_tcp_port):
    mock = MockServer(b'SPAMD/1.5 0 EX_OK\r\n')
    server = await asyncio.start_server(mock.handle_connection,
                                        host=HOST,
                                        port=unused_tcp_port,
