...
class FileEditor:
    def __init__(self, filename):
        ...
        if not self.filename.exists():
            raise FileExistsError(
                f"{self.filename} does not exist.  Check your file path."
            )
    ...
    def run(self, new_version):
        ...
        edited_contents = [
            self.edit(line, new_version) for line in self.read_contents()
        ]
...
def main():
    ...
    parser.add_argument(
        "-p", "--path", default=".", help="Path to the root of the project."
    )
    parser.add_argument(
        "-v",
        "--verbose",
    ...
    files = [
        AiospamcInit(project_path),
        SphinxDocs(project_path),
        PyProject(project_path),
    ]
    for file in files:
